<template>
  <div class="container">
    ...
    <div class="upload-section">
      ...
      <input type="file" @change="e => file = e.target.files[0]" accept=".zip" />
...
let es = null
const fetchDeps = async () => deps.value = await (await fetch(`${api}/deployments`)).json()
const upload = async () => {
  if (!file.value || !name.value) return
  up.value = true
  const fd = new FormData(); fd.append('file', file.value); fd.append('name', name.value)
  await fetch(`${api}/upload`, { method: 'POST', body: fd })
  up.value = false; name.value = ''; file.value = null; fetchDeps()
}
const viewLogs = id => {
  if (es) es.close()
